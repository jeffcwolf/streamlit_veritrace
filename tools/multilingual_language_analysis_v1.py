...
# Function to perform OCR on a PDF (placeholder - you'll need to implement
# this with an OCR library)
...
def detect_language(text, lingua_detector):
    ...
    if lingua_result and langdetect_result:
        ...
        # Only langdetect detected, slightly lower confidence
        return Language[langdetect_result.upper()], 0.6
    else:
...
def segment_languages_ngram(text, lingua_detector, n=3):
    ...
    # Ensure total_segments is at least 1
    total_segments = max(1, len(words) // n)
...
def segment_languages_lines(text, lingua_detector, n=3):
    ...
    progress_bar = st.progress(0)
    # Ensure total_segments is at least 1
    total_segments = max(1, len(lines) // n)
...
def generate_language_report(segmented):
    ...
    language_counts = Counter(
        lang.name if lang else "Unknown" for _,
        lang,
        _ in segmented)
    ...
    for i, (chunk, lang, confidence) in enumerate(
            segmented[:10], 1):  # Show first 10 segments
        report += f"Segment {i}:\n"
...
def main():
    st.title("VERITRACE Labs - Enhanced PDF Language Analyzer")
    ...
    if uploaded_file is not None:
        ...
        file_size = os.path.getsize(
            tmp_file_path) / (1024 * 1024)  # Size in MB
        ...
        analysis_method = st.selectbox(
            "Choose analysis method", [
                "ngram", "lines"])
        n = st.slider("Select n-gram size or number of lines",
                      min_value=1, max_value=10, value=3)

        if st.button("Analyze"):
            with st.spinner('Analyzing languages...'):
                # Detect languages
                language_spans = detect_languages(
                    text, method=analysis_method, n=n)
                ...
                if not language_spans:
                    ...
                    lang_stats = Counter(
                        lang.name if lang else "Unknown" for _, lang, _ in language_spans)
                    fig, ax = plt.subplots()
                    sns.barplot(
                        x=list(
                            lang_stats.keys()), y=list(
                            lang_stats.values()), ax=ax)
